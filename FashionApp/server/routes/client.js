...
class clientApplication {
    async submitTxn(
        ...
        try {
            ...
            } else if (txnType == "dressTxn") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            }else if (txnType == "approveTxn") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            }else if (txnType == "readDressTxn") {
                resultBytes = await contract.evaluateTransaction(txnName, ...args);
            }else if (txnType == "readHistoryTxn") {
                resultBytes = await contract.evaluateTransaction(txnName, ...args);
            }else if (txnType == "orderDressTxn") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            }else if (txnType == "privateNewTxn") {
                resultBytes= await contract.submit(txnName, {
                    arguments: [...args],
                    transientData: transientData,
                    endorsingOrganizations : ['retailerMSP','supplierMSP','manufacturerMSP']
                });
            }else if (txnType == "orderTxn") {
